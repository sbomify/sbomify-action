...
class TestWhenItCounts:
    ...
    @pytest.mark.parametrize("manifest", ["pyproject.toml", "package.json", "Cargo.toml", "pom.xml", "build.gradle"])
...
class TestACommittedResolutionIsRecognised:
    ...
    @pytest.mark.parametrize("manifest", ["Cargo.toml", "mix.exs", "stack.yaml"])
...
class TestDecidedByContentNotByName:
    """Two files where the name says nothing about whether a resolution was
    recorded, so the contents decide."""

    def test_go_mod_is_a_recorded_resolution(self, tmp_path):
        """go.mod cannot express a range -- the format takes an exact version,
        and Go resolves by minimal version selection against an immutable
        proxy. Measured on kubernetes v1.35.1: 207 of 207 requires carry an
        exact semver and the file holds no range operator at all. Treating it
        as inferred understated the strongest manifest in the corpus."""
        (tmp_path / "go.mod").write_text("module x\n\nrequire github.com/a/b v1.2.3\n")

        assert not resolution_was_inferred(str(tmp_path / "go.mod"))

    def test_go_mod_needs_no_go_sum_to_be_believed(self, tmp_path):
        """go.sum verifies hashes; it does not decide versions. Using its
        presence as the signal for "recorded" was the wrong file for the job."""
        (tmp_path / "go.mod").write_text("module x\n")

        assert not resolution_was_inferred(str(tmp_path / "go.mod"))

    def test_a_fully_pinned_requirements_txt_is_a_record(self, tmp_path):
    ...
    def test_anything_less_than_fully_pinned_is_inferred(self, tmp_path, contents):
        (tmp_path / "requirements.txt").write_text(contents)

        assert resolution_was_inferred(str(tmp_path / "requirements.txt"))

    def test_comments_and_options_do_not_count_against_it(self, tmp_path):
    ...
    def test_environment_markers_do_not_count_against_it(self, tmp_path):
