...
# Version used only when the running package can't report its own (e.g. a dev
# checkout with no installed metadata, where ``__version__`` is "unknown").
# Emitted workflows still get a tag-pinned ref — and there's no SHA to drift.
_FALLBACK_ACTION_VERSION = "v26.2.0"
...
def _action_version() -> str:
    ...
    Falls back to ``_FALLBACK_ACTION_VERSION`` when the package can't
    report its own version (``"unknown"`` from a metadata-less checkout).
    """
    version = (_PACKAGE_VERSION or "").strip()
    if version and version != "unknown":
        return f"v{version}"
    return _FALLBACK_ACTION_VERSION
